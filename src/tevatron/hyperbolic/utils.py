...
def clip_text_max_length(tokenizer, data_args_max_len: Optional[int] = None) -> int:
    """CLIP text encoders are capped at 77 tokens; never use VLM-style max_len blindly."""
    cap = getattr(tokenizer, "model_max_length", 77)
    if cap is None or cap > 10_000:
        cap = 77
    if data_args_max_len is not None:
        cap = min(cap, data_args_max_len)
    return int(cap)
