...
# =============================================================================
# Embedder Utility Class
# =============================================================================

class Qwen3VLEmbedder:
    """
    Utility class for generating embeddings from Qwen3-VL models.
    
    This class provides a simple interface for encoding text, images, and videos
    into embeddings without needing to manage the full training pipeline.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            max_length: Maximum sequence length.
            min_pixels: Minimum pixels for images.
            max_pixels: Maximum pixels for images.
            total_pixels: Maximum total pixels for videos.
            fps: Frames per second for video.
            max_frames: Maximum frames to sample.
    ...
    def encode(
        ...
        for i in range(0, len(inputs), batch_size):
            ...
            embeddings = self._process_batch(batch_inputs, normalize)
    ...
    def _format_model_input(
        ...
        """Format single input into conversation format."""
        content = []
        conversation = [
    ...
    def _preprocess_inputs(
        ...
        """Preprocess conversations for model."""
        text = self.processor.apply_chat_template(
            conversations, add_generation_prompt=True, tokenize=False
        )
        
        try:
            images, _ = process_vision_info(conversations, image_patch_size=16)
        except Exception as e:
            logger.error(f"Error in processing vision info: {e}")
            images = None
            text = self.processor.apply_chat_template(
    ...
    @staticmethod
    def _pooling_last(hidden_state: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        """Pool embeddings using last valid token."""
        flipped_mask = attention_mask.flip(dims=[1])
        last_one_positions = flipped_mask.argmax(dim=1)
        col = attention_mask.shape[1] - last_one_positions - 1
        row = torch.arange(hidden_state.shape[0], device=hidden_state.device)
